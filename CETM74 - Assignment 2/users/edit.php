...
$userid = $_SESSION["id"];
$cid = "";
$category = $subcategory = $loc = $priority = $casesub = $casedesc = $status ="";
...
if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["cid"])){
    $cid =  trim($_GET["cid"]);
        ...
        // Prepare an insert statement
       $sql = "SELECT category,subcategory,loc,priority,casesub,casedesc,statu FROM case_tbl WHERE caseid = ?";
       ...
       if($stmt = mysqli_prepare($conn, $sql)){
            ...
            // Bind query result variables
            mysqli_stmt_bind_result($stmt, $category, $subcategory, $loc, $priority, $casesub, $casedesc, $status);
            mysqli_stmt_fetch($stmt);
        } else{
        $notification = "<div class='error-box' style='text-align:center'> Oops! Something went wrong. Please try again later.</div>";
}
